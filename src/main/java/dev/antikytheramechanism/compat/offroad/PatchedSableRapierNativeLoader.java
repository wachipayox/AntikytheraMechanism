...
/**
 * Loads the Sable 2.0.3 Rapier native patched for the Offroad continuous-force experiment.
 *
 * <p>The patched library is byte-for-byte the normal Sable Rapier backend apart from one additional
 * JNI entry point used by {@link OffroadNativeForceBridge}. Windows and Linux x86-64 builds bundled
 * with Antikythera were produced from the exact Sable 2.0.3 source used by the runtime dependency.</p>
 */
public final class PatchedSableRapierNativeLoader {
    ...
    public static synchronized boolean tryLoadPatchedNative() {
        ...
        try (InputStream input = PatchedSableRapierNativeLoader.class.getResourceAsStream(resourcePath)) {
            ...
            loaded = true;
            failure = "none";
            LOGGER.info("Loaded Antikythera patched Sable 2.0.3 Rapier native '{}' for continuous-force diagnosis", nativeName);
            return true;
        } catch (Throwable throwable) {
            failure = throwable.getClass().getSimpleName() + ": " + String.valueOf(throwable.getMessage());
            LOGGER.warn("Could not load Antikythera patched Sable 2.0.3 Rapier native '{}'; falling back to Sable stock native", nativeName, throwable);
            loaded = false;
            return false;
        }
    }

...
/** Client-only plot lookup and half-scale Catnip ghost rendering for Transmission Box MICRO ports. */
public final class TransmissionBoxMiniPlacementClient
        implements TransmissionBoxMiniPlacementHelper.ClientBridge {
    private static final TransmissionBoxMiniPlacementClient INSTANCE =
            new TransmissionBoxMiniPlacementClient();
    private static final Object GHOST_SLOT = new Object();

    private TransmissionBoxMiniPlacementClient() {
    }

    public static void register() {
        TransmissionBoxMiniPlacementHelper.registerClientBridge(INSTANCE);
    }
    ...
    public @Nullable BlockPos resolveManagedPlotTarget(
        ...
        for (SubLevel candidate : container.getAllSubLevels()) {
            ...
            if (match != null && !match.equals(resolved)) {
                // Tracking should never expose two live children for one assembly. Refuse to guess
                // during a transient duplicate rather than previewing into the wrong plot.
                return null;
            }
    ...
    @Override
    public void renderPreview(TransmissionBoxMiniPlacementHelper.Preview preview) {
        ...
        BlockState physicalState = preview.logicalGhostState().rotate(switch (preview.sourceOrientation().front()) {
            case NORTH -> Rotation.NONE;
            case EAST -> Rotation.CLOCKWISE_90;
            case SOUTH -> Rotation.CLOCKWISE_180;
            case WEST -> Rotation.COUNTERCLOCKWISE_90;
            default -> throw new IllegalStateException(
                    "Static Frame front is not horizontal: " + preview.sourceOrientation().front());
        });
    ...
    private static final class HalfScaleGhostRenderer extends GhostBlockRenderer {
        ...
        public void render(
            ...
            // Match Create's transparent placement guide shrink, but inside one half-scale cell.
